...
mid :: Int -> Int -> Int -> Int
mid x y z
    | (x > y && y > z) || (x < y && y < z) || (x == y) = y
    | (y > x && x > z) || (y < x && x < z) || (x == z) = x
    | otherwise = z
...
sumLast2 n =
    let m = abs n
        lastDigit = mod m 10
        noLastDigit = div (m - lastDigit) 10
        secondToLastDigit = mod noLastDigit 10
    in secondToLastDigit + lastDigit
...
firstDigit n
    | m < 10 = m
    | otherwise = firstDigit noLastDigit
    where m = abs n
          lastDigit = mod m 10
          noLastDigit = div (m - lastDigit) 10
